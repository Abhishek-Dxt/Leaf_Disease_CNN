...
st.set_page_config(page_title='Leaf Disease Detection using CNN')
st.markdown("<h2 style='font-family:georgia; text-align: center; color:#4d7902' > Leaf Disease Detection using CNN </h1>", unsafe_allow_html=True)
...
st.header('About')
about = """
            This application uses a model trained using a 16-Layer Convolutional Neural Network to estimate if a leaf has any of the common leaf diseases.
            The leaf images can be uploaded from storage or can be uploaded using a phone camera.
            The dataset used to train the model can be found here - https://www.kaggle.com/datasets/vipoooool/new-plant-diseases-dataset \n
            Check my other projects at - https://abhishek-dxt.github.io/
"""
st.write(about)